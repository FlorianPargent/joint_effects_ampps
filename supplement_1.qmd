...
We now want to demonstrate how one could use the sample we just "collected" to estimate the expected potential outcomes we specified earlier. For estimation, we use Bayesian regression models fit with the {brms} package to compute the parametric g-formula. The correctly specified regression model based on our assumed data generating process looks as follows:
...
As we describe in the manuscript, a an average joint effect is a difference between two expected potential outcomes, e.g.:

$E(Y^{a_1=1, a_2=1} - Y^{a_1=0, a_2=0}) = E(Y^{a_1=1, a_2=1}) - E(Y^{a_1=0, a_2=0}) = `r E_Y_11 - E_Y_00`$
