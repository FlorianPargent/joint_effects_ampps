...
We now want to demonstrate how one could use the sample we just "collected" to estimate the expected potential outcomes we specified earlier. Potential Outcomes adn causal effects can be retrieved under the assumptions of consistency, exchangeability and positivity. For treatment regimes with interventions at both time points, we ensure exchangeability by leveraging the property that, given $L$, $A_2$ is independent of $U$: $P(A_2 = a_2 | L, U) = P(A_2 = a_2 | L)$.
